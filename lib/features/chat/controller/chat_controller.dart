...
class ChatController extends GetxController {
  ...
  void _loadCachedMessages() {
    if (_chatBox.containsKey(roomId.value) && messages.isEmpty) {
      ...
      messages.value = cachedMessages.reversed
          .toList(); // Load in reverse order for bottom-newest
      if (messages.isNotEmpty) {
        lastMessageId.value =
            messages.first['id']; // Use first ID for pagination (oldest)
      }
    }
  }
      ...
      switch (decodedMessage['type']) {
        case 'loadMessages':
          ...
          if (conversation != null) {
            ...
            final allMessages =
                ...
                [];
            messages.value = allMessages.reversed
                .toList(); // Reverse for bottom-newest display
            if (messages.isNotEmpty) {
              lastMessageId.value =
                  messages.first['id']; // Oldest message ID for next load
            }
        ...
        case 'loadMoreMessagesResponse':
          ...
          if (newMessages.isNotEmpty) {
            ...
            lastMessageId.value =
                newMessages.first['id']; // Update with oldest message ID
            _saveMessagesToCache();
          }
          break;
  ...
  void _addMessage(
    ...
    if (!messages.any((msg) => msg['id'] == messageId)) {
      ...
      debugPrint(
        "💬 Added message: $content (${fileUrl.isNotEmpty ? '📎 with file' : ''})",
      );
    }
  }
